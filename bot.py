...
# === Поиск товаров — через резервный прокси (работает в 2025) ===
def search_wb(query: str) -> list:
    ...
    proxy_url = f"https://wildberries-api-proxy.netlify.app/api/search?q={encoded_query}"
    ...
    try:
        response = requests.get(
            ...
            headers={
                ...
                "Referer": "https://wildberries-api-proxy.netlify.app"
            }
        )
        ...
        for p in products[:50]:  # Берём максимум 50
            try:
                pid = int(p.get("id"))
                ...
                continue  # Пропускаем битые товары
        ...
        return None  # Чтобы сработал фолбэк
